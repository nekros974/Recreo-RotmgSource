...
namespace wServer.realm.entities
{
    partial class Player
    {
                ...
                if (slot < cInv.Length)
                {
                    ...
                    if (item.Ranger)
                    {
                        ...
                        task.ContinueWith(t =>
                        {
                            ...
                            Activate(time, item, pos);
                           // if (Stats.Owner.HP <= Stats[0]/2) //cool shit
                           // {
                       ...
                       //         SendError("stat 0: " + Stats[0] + "stat 0 stats: " + Stats.Owner.HP);
            ...
            foreach (var eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.StasisBlast:
                        ...
                        break;
                    case ActivateEffects.SlowBlast:
                        SlowBlast(time, item, target, eff);
                        break;
                    case ActivateEffects.Decoy:
                        AEDecoy(time, item, target, eff);
                        break;
                    case ActivateEffects.Summon:
                        AESummonAbility(time, item, target, eff);
                        break;
                    ...
                    case ActivateEffects.ShurikenAbility:
                        ...
                        break;
                    case ActivateEffects.WarlockAbility:
                        AEWarlockAbility(time, item, target, eff);
                        break;
                    ...
                    case ActivateEffects.HealingGrenade:
                        ...
                        break;
                    case ActivateEffects.EventPointsActivate:
                        AEEventPointsActivate(time, item, target, eff);
                        break;
                    case ActivateEffects.RandomEventPoints:
                        AERandomEventPoints(time, item, target, eff);
                        break;
                    case ActivateEffects.RandomEventPoints2:
                        AERandomEventPoints2(time, item, target, eff);
                        break;
                    case ActivateEffects.CharSlot:
                        AECharSlot(time, item, target, eff);
                        break;
                    case ActivateEffects.VaultNum:
                        AEVaultNum(time, item, target, eff);
                        break;
                    default:
        ...
        private void AEWarlockAbility(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
        ...
        private void AESummonAbility(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var gameData = Manager.Resources.GameData;
           ...
           if (!gameData.IdToObjectType.TryGetValue(eff.Id, out objType))
            ...
            Entity en = Entity.Resolve(Manager, objType);
            en.Move(X, Y);
            Owner.EnterWorld(en);
            Owner.Timers.Add(new WorldTimer(30 * 250, (w, t) =>//600
            {
                w.LeaveWorld(en);
            }));
        ...
        private void AEEventPointsActivate(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var acc = Client.Account;
            Client.Manager.Database.UpdateEventPoints(acc, eff.Amount);
            EventPoints += eff.Amount;
            this.ForceUpdate(Credits);

        }

        private void AERandomEventPoints(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var acc = Client.Account;
            int EventPointsChance = Random.Next(0, 5);
            switch (EventPointsChance)
            {
                case 0:
                    Client.Manager.Database.UpdateEventPoints(acc, 1);
                    EventPoints += 1;
                    this.ForceUpdate(EventPoints);
                    SendInfo("You have obtained an Event Point.");
                    break;
        ...
        private void AECharSlot(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var acc = Client.Account;
            int SlotChance = Random.Next(0, 5);
            switch (SlotChance)
            {
                case 0:
                    Client.Manager.Database.UpdateCharSlot(acc, 1);
                   // EventPoints += 1;
                  //  this.ForceUpdate(EventPoints);
                    SendInfo("You have obtained a character slot.");
                    break;
        ...
        private void AEVaultNum(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var acc = Client.Account;
            int SlotChance = Random.Next(0, 5);
            switch (SlotChance)
            {
                case 0:
                    Client.Manager.Database.UpdateVaultNum(acc, 1);
                    // EventPoints += 1;
                    //  this.ForceUpdate(EventPoints);
                    SendInfo("You have obtained a vault chest.");
                    break;
        ...
        private void AERandomEventPoints2(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var acc = Client.Account;
            int EventPointsChance = Random.Next(0, 5);
            switch (EventPointsChance)
            {
                case 0:
                    Client.Manager.Database.UpdateEventPoints(acc, 10);
                    EventPoints += 10;
                    this.ForceUpdate(EventPoints);
                    SendInfo("You have obtained 10 Event Point.");
                    break;
        ...
        private void SlowBlast(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var pkts = new List<Packet>
            {
                new ShowEffect()
            ...
            Owner.AOE(target, 3, false, enemy =>
            {
                if (enemy.HasConditionEffect(ConditionEffects.SlowedImmune))
                {
                    pkts.Add(new Notification()
                    {
                        ObjectId = enemy.Id,
                ...
                else if (!enemy.HasConditionEffect(ConditionEffects.Slowed))
                {
                    enemy.ApplyConditionEffect(ConditionEffectIndex.Slowed, eff.DurationMS);

                    Owner.Timers.Add(new WorldTimer(eff.DurationMS, (world, t) =>
                        enemy.ApplyConditionEffect(ConditionEffectIndex.SlowedImmune, 3000)));
