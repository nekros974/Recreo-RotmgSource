...
{
    class DropPortalOnDeathNoTime : Behavior
    {
        private readonly ushort _target;
        private readonly float _probability;
        private readonly int? _timeout;

        public DropPortalOnDeathNoTime(string target, double probability = 1, int? timeout = null)
        {
        ...
        protected internal override void Resolve(State parent)
        {
            parent.Death += (sender, e) =>
            {
                    ...
                    Random.NextDouble() < _probability)
                {
                    ...
                    if (timeoutTime != 0)
                        owner.Timers.Add(new WorldTimer(timeoutTime * 100000, (world, t) => //default portal close time * 1000
                        {
                            try
                            {
