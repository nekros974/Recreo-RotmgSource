    ...
    public class Database : IDisposable
    {
        ...
        public DbAccount CreateGuestAccount(string uuid)
        {
            ...
            var acnt = new DbAccount(_db, 0)
            {
                ...
                EventPoints = newAccounts.EventPoints,
                TotalEventPoints = newAccounts.EventPoints,
                PassResetToken = ""
            };
        ...
        private static readonly Dictionary<CurrencyType, string[]> CurrencyKey = new Dictionary<CurrencyType, string[]>
        {
            ...
            { CurrencyType.EventPoints, new [] { "totalEventPoints", "eventPoints" } }
        };
        ...
        private void UpdateAccountCurrency(DbAccount acc, CurrencyType type, int value, bool total = false)
        {
            switch (type)
            {
                ...
                case CurrencyType.EventPoints:
                    if (total)
                        acc.TotalEventPoints = value;
                    else
                        acc.EventPoints = value;
                    break;
            }
        }
        ...
        public Task UpdateEventPoints(DbAccount acc, int amount, ITransaction transaction = null)
        {
            var trans = transaction ?? _db.CreateTransaction();

            if (amount > 0)
                trans.HashIncrementAsync(acc.Key, "totalEventPoints", amount)
                    .ContinueWith(t =>
                    {
                        if (!t.IsCanceled)
                            acc.TotalEventPoints = (int)t.Result;
                    });

            var task = trans.HashIncrementAsync(acc.Key, "eventPoints", amount)
                .ContinueWith(t =>
                {
                    if (!t.IsCanceled)
                        acc.EventPoints = (int)t.Result;
                });
        ...
        public Task UpdateCharSlot(DbAccount acc, int amount, ITransaction transaction = null)
        {
            var trans = transaction ?? _db.CreateTransaction();

            if (amount > 0)
                trans.HashIncrementAsync(acc.Key, "maxCharSlot", 0)
                    .ContinueWith(t =>
                    {
                        if (!t.IsCanceled)
                            acc.MaxCharSlot = (int)t.Result;
                    });

           var task = trans.HashIncrementAsync(acc.Key, "maxCharSlot", amount)
                .ContinueWith(t =>
                {
                    if (!t.IsCanceled)
                       acc.MaxCharSlot = (int)t.Result;
        ...
        public Task UpdateVaultNum(DbAccount acc, int amount, ITransaction transaction = null)
        {
            var trans = transaction ?? _db.CreateTransaction();

            if (amount > 0)
                trans.HashIncrementAsync(acc.Key, "vaultCount", 0)
                    .ContinueWith(t =>
                    {
                        if (!t.IsCanceled)
                            acc.VaultCount = (int)t.Result;
                    });

            var task = trans.HashIncrementAsync(acc.Key, "vaultCount", amount)
                 .ContinueWith(t =>
                 {
                     if (!t.IsCanceled)
                         acc.VaultCount = (int)t.Result;
        ...
        public void WipeAccount(DbAccount acc, XmlData dat, string killer)
        {
            ...
            _db.HashSetAsync(acc.Key, "eventPoints", 0, flags: CommandFlags.FireAndForget);
            _db.HashSetAsync(acc.Key, "totalEventPoints", 0, flags: CommandFlags.FireAndForget);
