...
namespace common
{
    public class FameStats
    {
        ...
        static Tuple<string, string, Func<FameStats, DbChar, int, bool>, Func<double, int>>[] bonusDat = new[] {
            ...
            Tuple.Create("Silver", "Silver Donator",
                new Func<FameStats, DbChar, int, bool>(
                    (fStats, character, baseFame) =>
                character.Account.Rank == 20 || character.Account.Rank == 30),
                new Func<double, int>(f => (int) (f * 0.05))
            ),

            Tuple.Create("Platinum", "Platinum Donator",
                new Func<FameStats, DbChar, int, bool>(
                    (fStats, character, baseFame) =>
                character.Account.Rank == 40),
                new Func<double, int>(f => (int) (f * 0.1))
            ),

            Tuple.Create("Diamond", "Diamond Donator",
                new Func<FameStats, DbChar, int, bool>(
                    (fStats, character, baseFame) =>
                character.Account.Rank == 50),
                new Func<double, int>(f => (int) (f * 0.15))
            ),

            Tuple.Create("Onyx", "Onyx Donator",
                new Func<FameStats, DbChar, int, bool>(
                    (fStats, character, baseFame) =>
                character.Account.Rank == 60),
                new Func<double, int>(f => (int) (f * 0.25))
            ),

            Tuple.Create("Eternal", "Eternal Donator",
                new Func<FameStats, DbChar, int, bool>(
                    (fStats, character, baseFame) =>
                character.Account.Rank == 70),
                new Func<double, int>(f => (int) (f * 0.5))
            ),
        };
